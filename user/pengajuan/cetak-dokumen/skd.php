<?php 
    ...
    if ($result2 && mysqli_num_rows($result2) > 0) {
        ...
        $nomor_skd = $data_pengajuan['id_pengajuan'];
        $nomor_surat_berikutnya_padded = str_pad($nomor_skd, 3, '0', STR_PAD_LEFT);
    } else {
    ...
    $pdf->Cell(115, 7, 'SURAT KETERANGAN DOMISILI', 0, 1, 'C');
    $pdf->SetLineWidth(0.5); // Ketebalan garis (dalam mm)
    $pdf->Line($textX + 10, $textY + 42, $textX + 98 + 7, $textY + 42); // Garis bawah sepanjang teks (dengan spasi tambahan)
    ...
    $startY += 7; // Tambahkan Y untuk baris berikutnya
    $pdf->SetXY($startX, $startY);
    $pdf->Cell(45, 7, 'Status Perkawinan', 0, 0, 'L'); // Teks Label
    $pdf->Cell(9, 7, '', 0, 0, 'L'); // Tanda ':'
    $pdf->Cell(5, 7, ':', 0, 0, 'L'); // Tanda ':'
    ...
    $keterangan2_awal = 'Benar bahwa Nama tersebut di atas adalah warga Bulak Lor, yang berdomisili ';
    $pdf->MultiCell(0, 7, $keterangan2_awal. $userData['alamat'].'.', 0, 'J');

    // penutup
    $pdf->SetFont('Times', '', 12);
    $pdf->SetXY($pdf->GetX(), $pdf->GetY() + 2);
    $keterangan = 'Demikian surat keterangan ini dibuat dengan sebenarnya dan dan dapat dipergunakan sebagaimana mestinya.';
    $pdf->MultiCell(0, 7, $keterangan, 0, 'J');
