<?php 
    ...
    $jumlah_sk = "SELECT COUNT(*) AS total FROM pengajuan where id_kategori = 3";
    ...
    $nama_file = 'SKTM_' . $userData['nama'] . '_' . $tanggal_acc . '.pdf';
    ...
    $pdf->Cell(115, 7, ' SURAT KETERANGAN TIDAK MAMPU', 0, 1, 'C');
    $pdf->SetLineWidth(0.5); // Ketebalan garis (dalam mm)
    $pdf->Line($textX + 4, $textY + 42, $textX + 105 + 7, $textY + 42); // Garis bawah sepanjang teks (dengan spasi tambahan)
    ...
    $keterangan2_awal = 'Benar bahwa nama tersebut di atas adalah warga masyarakat kami dan menurut sepengetahuan nama tersebut di atas tergolong Keluarga Ekonomi Lemah / Tidak Mampu, dan DTKS masih dalam proses. Surat keterangan tidak mampu ini : ';
    $pdf->MultiCell(0, 7, $keterangan2_awal, 0, 'J');
