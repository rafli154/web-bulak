<?php 
    ...
    if ($result2 && mysqli_num_rows($result2) > 0) {
        ...
        $nama_ortu = $data_pengajuan['nama_ortu'];
        $nik_ortu = $data_pengajuan['nik_ortu'];
        $pekerjaan_ortu = $data_pengajuan['pekerjaan_ortu'];
        $penghasilan = $data_pengajuan['penghasilan'];
        ...
        $nomor_skbn = $data_pengajuan['id_pengajuan'];
        $nomor_surat_berikutnya_padded = str_pad($nomor_skbn, 3, '0', STR_PAD_LEFT);
    } else {
    ...
    $jumlah_sk = "SELECT COUNT(*) AS total FROM pengajuan where id_kategori = 7";
    ...
    $nama_file = 'SKPOT' . '_' . $userData['nama'] . '_' . $tanggal_acc . '.pdf';
    ...
    $pdf->Cell(115, 7, 'SURAT KETERANGAN PENGHASILAN ORANG TUA', 0, 1, 'C');
    $pdf->SetLineWidth(0.5); // Ketebalan garis (dalam mm)
    $pdf->Line($textX - 15, $textY + 42, $textX + 125 + 5, $textY + 42); // Garis bawah sepanjang teks (dengan spasi tambahan)
    ...
    $pdf->SetFont('Times', 'B', 12);
    $pdf->Cell(0, 7, $nama_ortu, 0, 1, 'J'); // Data
    ...
    $pdf->Cell(9, 7, '', 0, 0, 'L'); // Tanda ':'
    $pdf->Cell(5, 7, ':', 0, 0, 'L'); // Tanda ':'
    $pdf->Cell(0, 7, $nik_ortu, 0, 1, 'L'); // Data
    ...
    $pdf->Cell(9, 7, '', 0, 0, 'L'); // Tanda ':'
    $pdf->Cell(5, 7, ':', 0, 0, 'L'); // Tanda ':'
    $pdf->Cell(0, 7, $pekerjaan_ortu, 0, 1, 'L'); // Data
    ...
    $pdf->SetFont('Times', 'B', 12);
    $pdf->SetXY($pdf->GetX(), $pdf->GetY() + 2);
    $keterangan = 'Data Anak :';
    $pdf->MultiCell(0, 7, $keterangan, 0, 'J');

    // Menentukan posisi untuk tabel data diri
    $startX = 30; // Posisi X
    $startY = 136; // Posisi Y, sesuaikan sesuai kebutuhan
    ...
    $pdf->Cell(5, 7, ':', 0, 0, 'L'); // Tanda ':'
    $pdf->SetFont('Times', 'B', 12);
    $pdf->Cell(0, 7, $nama_kk, 0, 1, 'L'); // Data
    ...
    $keterangan2 = "Menerangkan bahwa Orang Tua tersebut diatas menurut sepengetahuan kami, orang tersebut termasuk dalam kategori Kurang Mampu / Prasejahtera dan mempunyai penghasilan Rp. {$penghasilan} per bulannya.";
    $pdf->MultiCell(0, 7, $keterangan2, 0, 'J');

    // keterangan 2
    $pdf->SetFont('Times', '', 12);
    $pdf->SetXY($pdf->GetX(), $pdf->GetY() + 5);
    $keterangan3 = 'Demikian surat keterangan ini kami buat dengan sebenarnya dan untuk dipergunakan sebagaimana mestinya.';
    $pdf->MultiCell(0, 7, $keterangan3, 0, 'J');
