<?php 
    ...
    if ($result2 && mysqli_num_rows($result2) > 0) {
        ...
        $nomor_sk = $data_pengajuan['id_pengajuan'];
        $nomor_surat_berikutnya_padded = str_pad($nomor_sk, 3, '0', STR_PAD_LEFT);
    } else {
    ...
    // Menghitung jumlah keseluruhan data pengajuan sk
    $jumlah_sk = "SELECT COUNT(*) AS total FROM pengajuan where id_kategori = 6";
    ...
    $nama_file = 'SK_' . $userData['nama'] . '_' . $tanggal_acc . '.pdf';
    ...
    $pdf->Cell(115, 7, 'SURAT KETERANGAN', 0, 1, 'C');
    $pdf->SetLineWidth(0.5); // Ketebalan garis (dalam mm)
    $pdf->Line($textX + 25, $textY + 42, $textX + 85 + 5, $textY + 42); // Garis bawah sepanjang teks (dengan spasi tambahan)
    ...
    $keterangan = ' Yang  bertanda   tangan   di  bawah  ini,  Kuwu   Desa  Bulak  Kecamatan Jatibarang Kabupaten Indramayu, menerangkan bahwa :';
    ...
    $pdf->MultiCell(0, 7, 'Yang bersangkutan adalah Warga Desa Bulak Kecamatan Jatibarang, benar bahwa Nama tersebut diatas berstatus: ', 0, 'J');
    $pdf->SetFont('Times', 'B', 12);
    $pdf->MultiCell(0, 7, $userData['status_pernikahan'], 0, 'C');

    // penutup
    $pdf->SetFont('Times', '', 12);
    $pdf->SetXY($pdf->GetX(), $pdf->GetY() + 2);
    $keterangan = 'Demikian surat keterangan ini dibuat dengan sebenarnya dan dan dapat dipergunakan sebagaimana perlunya.';
    $pdf->MultiCell(0, 7, $keterangan, 0, 'J');
